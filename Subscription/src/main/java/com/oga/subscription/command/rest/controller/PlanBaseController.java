...
import com.oga.subscription.command.rest.response.CreatePlanResponse;
...
public class PlanBaseController {
    ...
    /**
     * Constructs a PlanBaseController instance.
     *
     * @param commandDispatcher The command dispatcher for sending commands.
     */
    @Autowired
    public PlanBaseController(CommandDispatcher commandDispatcher) {
     ...
     *
     * @param command The CreatePlanCommand object representing the product to create.
    ...
    public ResponseEntity<BaseResponse> createPlan(@RequestBody CreatePlanCommand command) {
        var id = UUID.randomUUID().toString();
        command.setId(id);
        commandDispatcher.send(command);
        return new ResponseEntity<>(new CreatePlanResponse(Constant.SUCCESS_MESSAGE, command.getId()), HttpStatus.CREATED);
    }
    @DeleteMapping
    public ResponseEntity<BaseResponse> deletePlan() {
        DeleteCatalogCommand command = new DeleteCatalogCommand();
        command.setId(command.getIdentifier());
        commandDispatcher.send(command);
        return new ResponseEntity<>(new DeleteProductResponse(Constant.SUCCESS_MESSAGE, command.getIdentifier()), HttpStatus.OK);
    }
}
